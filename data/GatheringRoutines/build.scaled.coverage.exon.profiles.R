...
### build.scaled.coverage.exon.profiles ###


build.scaled.coverage.exon.profiles = function(
		gene.anno,
		bam.file.subsets,
		on,
		from,
  ...
  for (bam.file.subset in 1:length(bam.file.subsets)){
    ...
    for (cov.path in bam.file.subsets[[bam.file.subset]]){
        ...
        if(on == "antisense"){
          if(strand.transcript == "+"){
            ...
            if (length(valid.positions) > 0){
              cov.vec = Rle(resize.profile(as.vector(chr.coverage.list.from.bam[[strand.switch(strand.transcript)]][valid.positions])[as.logical(as.vector(chr.coverage.exon.list.from.bam[[strand.switch(strand.transcript)]][valid.positions]))],(median.length+1)))
          ...
          if(strand.transcript == "+"){
            ...
            if (length(valid.positions) > 0){
              cov.vec = Rle(resize.profile(as.vector(chr.coverage.list.from.bam[[strand.transcript]][valid.positions])[as.logical(as.vector(chr.coverage.exon.list.from.bam[[strand.transcript]][valid.positions]))],(median.length+1)))
              vec.template[c(rep(FALSE,length(upstream.positions)),rep(TRUE,(median.length+1)),rep(FALSE,length(downstream.positions)))] = cov.vec
            }
            if (length(valid.downstream.positions) > 0){
      ...
      for (index.subset in names(index.subsets)){
        ...
        chr.coverage.exon.list.from.bam = get(load(file.path("IndicatorRleTracks",paste0("exon.refseq.merge.indicator.rle.list.",index.subset,".RData"))))
