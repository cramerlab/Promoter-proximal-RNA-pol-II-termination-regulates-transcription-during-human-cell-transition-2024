...
### build.scaled.coverage.profiles ###
build.scaled.coverage.profiles = function(
		gene.anno,
		bam.file.subsets,
		on,
		from,
  ...
  for (bam.file.subset in 1:length(bam.file.subsets)){
    ...
    for (cov.path in bam.file.subsets[[bam.file.subset]]){
      ...
      build.coverage.list = function(j){
        ...
        length.transcript = chr.gene.anno[j,"width"]
        ...
        if(on == "antisense"){
          if(strand.transcript == "+"){
            ...
            if (length(valid.positions) > 0){
              cov.vec = Rle(resize.profile(as.vector(chr.coverage.list.from.bam[[strand.switch(strand.transcript)]][valid.positions]),(median.length+1)))
          ...
          if(strand.transcript == "+"){
            ...
            if (length(valid.positions) > 0){
              cov.vec = Rle(resize.profile(as.vector(chr.coverage.list.from.bam[[strand.transcript]][valid.positions]),(median.length+1)))
              vec.template[c(rep(FALSE,length(upstream.positions)),rep(TRUE,(median.length+1)),rep(FALSE,length(downstream.positions)))] = cov.vec
            }
            if (length(valid.downstream.positions) > 0){
      ...
      for (index.subset in names(index.subsets)){
        ...
        chr.gene.anno = gene.anno[which(gene.anno[,"chr"] == index.subset),c("start","end","strand","width")]
